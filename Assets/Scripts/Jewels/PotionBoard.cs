...
public class PotionBoard : MonoBehaviour
{
    ...
    private int rewardItemsEarned = 0;
    ...
    public bool CheckBoard()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                // Cek apakah node ini dapat digunakan
                if (potionBoard[x, y].isUsable)
                {
                    ...
                    if (!potion.isMatched)
                    {
                        ...
                        if (matchedPotions.connectedPotions.Count >= 3)
                        {
                            ...
                            int matchCount = superMatchedPotions.connectedPotions.Count;
        ...
        // Menambahkan reward ke inventory
        if (rewardItemsEarned > 0 && potionsToRemove.Count > 0)
        {
            PotionType matchedType = potionsToRemove[0].potionType;
            InventoryManager.Instance.AddItem(matchedType, rewardItemsEarned);
            Debug.Log("Reward Added: " + rewardItemsEarned + "x " + matchedType);

            rewardItemsEarned = 0; // Reset reward
        }
    ...
    public IEnumerator ProcessTurnOnMatchedBoard(bool _subtractMoves)
    {
        ...
        // Tambahkan ini sebelum delay atau sebelum selesai turn
        if (rewardItemsEarned > 0)
        {
            Debug.Log("You earned " + rewardItemsEarned + " item(s) from matching!");
            // TODO: Tambahkan ke inventory player di sini
        }
        rewardItemsEarned = 0; // Reset
